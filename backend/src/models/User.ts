...
UserSchema.pre("findOneAndDelete", async function (this: PreDeleteQueryMiddleware, next) {
    try {
        ...
        await UserLetter.deleteMany({ sender_id: userId });
        await UserLetter.deleteMany({ receiver_id: userId });

        await Friend.deleteMany({ user_id_1: userId })
        await Friend.deleteMany({ user_id_2: userId })
