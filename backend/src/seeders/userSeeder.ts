...
async function seedFriends() {
    ...
    if (!existingAdmin || !existingUser) {
        throw new Error("Error when adding friend relation: Admin or user inexistant")
    }
    const existingRelation = await Friend.findOne({
        $or: [
            {
                user_id_1: existingUser._id, user_id_2: existingAdmin._id
            },
            {
                user_id_2: existingUser._id, user_id_1: existingAdmin._id
            }
        ]
    })
    if (!existingRelation) {
        const relationCreaditential = {
            user_id_1: existingAdmin._id,
            user_id_2: existingUser._id
        }
        const relation = await Friend.create(relationCreaditential);
        console.log("created relation: ", relation)
    } else {
        console.warn("friend relation is already present in db, skipping: ", existingRelation)
    }
}
